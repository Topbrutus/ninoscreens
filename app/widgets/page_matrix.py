...
class PageMatrix(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._tiles_cache: list[TileState] = []
    ...
    def set_split_pairs(self, split_pairs: dict[int, int], tiles: list[TileState]) -> None:
        self._split_pairs = dict(split_pairs)
        self._tiles_cache = list(tiles)
        for slot_index, state in enumerate(tiles):
            self.set_slot_state(slot_index, state)

    def _compose_duo_icon(self, left_pixmap: QPixmap | None, right_pixmap: QPixmap | None) -> QIcon:
        canvas = QPixmap(52, 24)
        canvas.fill(Qt.GlobalColor.transparent)

        painter = QPainter(canvas)
        try:
            if left_pixmap is not None and not left_pixmap.isNull():
                left_scaled = left_pixmap.scaled(
                    20, 20,
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation,
                )
                painter.drawPixmap(4, 2, left_scaled)

            if right_pixmap is not None and not right_pixmap.isNull():
                right_scaled = right_pixmap.scaled(
                    20, 20,
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation,
                )
                painter.drawPixmap(28, 2, right_scaled)
        finally:
            painter.end()
    ...
    def set_slot_state(self, slot_index: int, state: TileState) -> None:
        ...
        partner_id = self._split_pairs.get(slot_index)
        if partner_id is not None and 0 <= partner_id < len(self._tiles_cache):
            partner_state = self._tiles_cache[partner_id]
            button.setIcon(self._compose_duo_icon(state.site_icon, partner_state.site_icon))
            button.setText("")
            button.setToolTip(
                f"Split {slot_index + 1} + {partner_id + 1} • "
                f"{state.display_title} + {partner_state.display_title}"
            )
        else:
            if state.site_icon is not None and not state.site_icon.isNull():
                button.setIcon(QIcon(state.site_icon))
                button.setText("")
            else:
